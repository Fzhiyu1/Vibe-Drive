...
| 方案 | 实现方式 | 结果 |
|------|----------|------|
| 场景Y轴旋转 | `scene.rotation.y` | ❌ "只是视角转动，路面没动" |
| 相机Z轴旋转 | `camera.rotation.z` | ❌ "整个世界都在倾斜" |
| 相机Y轴旋转 | `camera.rotation.y` | ❌ 效果不自然 |
| 视差效果 | 两侧物体不同移动速度 | ❌ 看不出效果 |

**视差效果方案详情**：

```typescript
// 转弯时内侧物体移动更快，外侧更慢
const sideFactor = tree.side === 'left'
  ? 1 - turnDirection * 0.5  // 左转时左侧快
  : 1 + turnDirection * 0.5  // 左转时右侧慢

tree.group.position.z += moveDistance * sideFactor
```

**失败原因分析**：
1. 固定视角下，视差效果不明显
2. 道路本身是直的，无法产生"弯道"感觉
3. 相机旋转会导致整个画面倾斜，不自然
